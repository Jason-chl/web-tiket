...
// Query to get all checked-in attendees
$query = "
    SELECT t.nama_pemegang, t.email_pemegang, t.waktu_checkin, e.nama_event, tk.nama_tiket, u.nama as petugas_nama
    FROM attendee t
    ...
    LEFT JOIN users u ON t.used_by_admin_id = u.id_user
    WHERE t.status_checkin = 'used'
";

if (!empty($search)) {
    $query .= " AND (t.nama_pemegang LIKE :s OR e.nama_event LIKE :s OR t.kode_tiket LIKE :s)";
}

$query .= " ORDER BY t.waktu_checkin DESC";
...
<head>
    ...
    <title>Daftar Hadir — TixNow</title>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .page-title { font-family: 'Syne', sans-serif; font-size: 2rem; font-weight: 800; color: var(--header-text); }
        
        .search-box { position: relative; width: 300px; }
        .search-input { width: 100%; background: var(--card); border: 1px solid var(--border); padding: 12px 18px 12px 40px; border-radius: 12px; color: var(--text); font-family: inherit; font-size: 0.85rem; }
        .search-input:focus { border-color: var(--accent); outline: none; }
        .search-icon { position: absolute; left: 14px; top: 50%; transform: translateY(-50%); color: var(--muted); }

        /* TABLE */
        .panel { background: var(--card); border: 1px solid var(--border); border-radius: 20px; overflow: hidden; box-shadow: 0 4px 20px rgba(0,0,0,0.1); }
        table { width: 100%; border-collapse: collapse; }
        th { text-align: left; padding: 16px 24px; font-size: 0.75rem; color: var(--muted); font-weight: 600; text-transform: uppercase; border-bottom: 1px solid var(--border); background: rgba(255,255,255,0.01); }
        td { padding: 18px 24px; font-size: 0.85rem; border-bottom: 1px solid var(--border); }
        tr:last-child td { border-bottom: none; }
        
        .user-cell { display: flex; flex-direction: column; }
        .user-name { font-weight: 700; color: var(--header-text); }
        ...
        .badge-event { background: var(--accent-glow); color: var(--accent); }
        .badge-petugas { background: rgba(255,255,255,0.05); color: var(--muted); }
...
<main class="main">
    <div class="page-header">
        <div>
            <h1 class="page-title">Daftar Hadir</h1>
            <p class="page-sub">Data penonton yang sudah masuk ke area event</p>
        </div>
        <form method="GET" class="search-box">
            ...
            <input type="text" name="search" class="search-input" placeholder="Cari nama atau event..." value="<?php echo htmlspecialchars($search); ?>">
        </form>
    </div>
            ...
            <tbody>
                <?php if (count($attendees) > 0): ?>
                    ...
                    <tr>
                        <td>
                            <div class="user-cell">
                                <span class="user-name"><?php echo htmlspecialchars($row['nama_pemegang']); ?></span>
                                <span class="user-email"><?php echo htmlspecialchars($row['email_pemegang']); ?></span>
                            </div>
                        </td>
                        <td>
                            <div style="font-weight:600; margin-bottom:4px;"><?php echo htmlspecialchars($row['nama_event']); ?></div>
                            <span class="badge badge-event"><?php echo htmlspecialchars($row['nama_tiket']); ?></span>
                        </td>
                        <td>
                            <div style="font-weight:600;"><?php echo date('d M Y', strtotime($row['waktu_checkin'])); ?></div>
                            <div style="font-size:0.75rem; color:var(--muted);"><?php echo date('H:i:s', strtotime($row['waktu_checkin'])); ?></div>
                        </td>
                        <td>
                            <span class="badge badge-petugas"><?php echo htmlspecialchars($row['petugas_nama'] ?? 'Sistem'); ?></span>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr>
                        <td colspan="4">
                            <div class="empty-state">
                                <svg width="48" height="48" fill="none" stroke="currentColor" viewBox="0 0 24 24" style="opacity:0.2; margin-bottom:15px;"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857"/></svg>
                                <p>Tidak ada data penonton ditemukan.</p>
                            </div>
                        </td>
                    </tr>
                <?php endif; ?>
